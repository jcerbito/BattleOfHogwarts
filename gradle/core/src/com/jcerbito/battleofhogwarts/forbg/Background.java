        ...
        for (int y = 0; y <= stage.getHeight(); y += Resources.TILE_SIZE){
            for (int x = 0; x <= stage.getWidth(); x += Resources.TILE_SIZE) {
                if (GameUpgradeEasy.currentLvl == 1 ){
                    ...
                    stage.getBatch().draw(res.tile7, x, y, 0, 0, Resources.TILE_SIZE, Resources.TILE_SIZE, 1.01f, 1.01f, 0);
                }
        ...
        for (int y = 0; y <= stage.getHeight(); y += Resources.TILE_SIZE) {
            for (int x = 0; x <= stage.getWidth(); x += Resources.TILE_SIZE) {
                if (GameUpgradeEasy.currentLvl == 1  ){
                    ...
                    stage.getBatch().draw(res.tile3, x, y + ((stage.getHeight() / 2) + 90), 0, 0, Resources.TILE_SIZE, Resources.TILE_SIZE, 1.00f, 1.00f, 0 ); //+ (Resources.TILE_SIZE + (stage.getHeight() / 2) + 68)
                }else if (GameUpgradeEasy.currentLvl == 3 ){
                    stage.getBatch().draw(res.tile6, x, y + ((stage.getHeight() / 2) + 90), 0, 0, Resources.TILE_SIZE, Resources.TILE_SIZE, 1.00f, 1.00f, 0 ); //+ (Resources.TILE_SIZE + (stage.getHeight() / 2) + 68)
                }
            }
        }
