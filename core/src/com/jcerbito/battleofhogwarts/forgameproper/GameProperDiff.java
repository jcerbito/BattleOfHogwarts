...
public class GameProperDiff implements EnemyDiff.EnemyDiffAttackedListener, LightningBoltFx.LightningBoltFxListener{
    ...
    private void randomEq(){
        ...
        equipments.add(Equipment.Create(lx, ly, MathUtils.random(3) == 0 ? Equipment.HEART : Equipment.WAND, game.res));
                ...
                if (currEquipment.getEq() == Equipment.HEART){
                    ...
                    if (enemy.getLives() <= 0){
                        if (GameUpgradeDiff.currentLvl == 1){
                            ...
                            GameUpgradeDiff.currentLvl = 4;
                            if (GameUpgradeDiff.currentLvl >= 4){
                                game.setScreen(new WinScreenDiff(game));
    ...
    public void OnEffect(LightningBoltFx effect) {
        if(effect.getLocX() == player.getLocX() && effect.getLocY() == player.getLocY()){
            ...
            }else if(player.getLives() <= 0 && GameUpgradeDiff.currentLvl == 2){
                GameUpgradeDiff.ResetSecondLevel();
                game.setScreen(new GameOverScreen(game));
            }else if(player.getLives() <= 0 && GameUpgradeDiff.currentLvl == 3){
                GameUpgradeDiff.ResetThirdLevel();
                game.setScreen(new GameOverScreen(game));
            }
        }
    }
